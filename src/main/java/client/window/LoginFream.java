	...
	public LoginFream(BeginWindow bWindow){
		...
		loninButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				new Thread(() -> {
					String reqStr = HttpConnectUtil.requestHTTP(BeginWindow.webHost +"/login",msg,"get");
					try {
						...
						if("true".equals(res.get("status"))) {
							try {
								...
								String msg1 = resJSON.getString("msg");
								if(msgType.equals("AlreadyLogined") && msg1.equals("0") ) {
		...
		regignButt.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					// 使用默认浏览器弹出窗口
					desktop.browse(new URI(BeginWindow.webHost +"/"));
